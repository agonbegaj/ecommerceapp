...
const prevPage = (e) => {
  ...
  if(prev_page < 1) alert ("Error you can't go lower!")
          ...
          { loading ? 'Loading movies ...' : movies && movies.map(movie => (
            <div className="product">
              <Link to={`/product/${movie.id}`}>
                {
                 (movie.poster_path === null || movie.poster_path ==="null" ) ? (<img src={noimage} alt={movie.original_title}/>) : (<img src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt={movie.original_title} />)
                }
              </Link>
